...
const Hero = () => {
  ...
  const rotateCount = heroBanners.length;

  useEffect(() => {
    if (rotateCount <= 1) return undefined;
    const id = setInterval(() => {
      setHeroIndex((i) => (i + 1) % rotateCount);
    }, 5500);
    return () => clearInterval(id);
  }, [rotateCount]);

  const main = heroBanners[heroIndex] || heroBanners[0];
  const side1 = heroBanners[(heroIndex + 1) % Math.max(rotateCount, 1)] || heroBanners[1];
  const side2 = heroBanners[(heroIndex + 2) % Math.max(rotateCount, 1)] || heroBanners[2];
  const promo = heroBanners[3];
  const trending = heroBanners[4];
    ...
    <section className="relative overflow-hidden" data-testid="hero-section">
      <div className="container-px mx-auto max-w-[1400px] pt-12 sm:pt-20 pb-16 sm:pb-24 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        <div className="lg:col-span-6 fade-up">
          ...
          <div className="mt-10 sm:mt-14 grid grid-cols-3 gap-4 sm:gap-8 max-w-md">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">Happy shoppers</p>
            </div>
            <div>
              <p className="font-display text-2xl sm:text-3xl font-bold">15K+</p>
              <p className="text-xs text-muted-foreground mt-1">Products curated</p>
            </div>
            <div>
        ...
        >
          <CtaLink
            to={main?.cta_link}
            className="col-span-4 row-span-4 relative overflow-hidden bg-muted block rounded-lg"
                ...
                {heroBanners.map((_, i) => (
                  <span
                    ...
                    className={`h-1.5 rounded-full transition-all ${
                      i === heroIndex ? "w-4 bg-white" : "w-1.5 bg-white/40"
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="col-span-2 row-span-2 relative overflow-hidden bg-muted rounded-lg">
            <img src={sideImg1} alt="" loading="eager" className="w-full h-full object-cover" />
          </div>
          <div className="col-span-2 row-span-2 relative overflow-hidden bg-muted rounded-lg">
            <img src={sideImg2} alt="" loading="eager" className="w-full h-full object-cover" />
          </div>
          <CtaLink
            to={promoLink}
            className="col-span-2 row-span-2 relative overflow-hidden bg-foreground text-background flex flex-col justify-end p-4 sm:p-5 rounded-lg"
          ...
          <CtaLink
            to={trendingLink}
            className="col-span-4 row-span-2 relative overflow-hidden bg-secondary flex items-center px-5 sm:px-8 rounded-lg"
